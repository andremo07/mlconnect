...
//		getLogger().trace("Iniciando gera��o das NFes");
	...
	public List<InputStream> generateNFePdf(List<NFNotaProcessada> notasProcessadas) throws NfeProviderException{
		...
		try {
			...
			//getLogger().trace("Gera��o das NFes finalizado");
	...
	public NFEndereco getNFEnderecoDest(Venda venda) {
		...
		//Criar rotina para carregar o c�digo do munic�pio na tabela de envio no momento da importa��o da venda. O cadastro dos c�digos est�o na tabela MUNICIPIO.
		...
		// Criar rotina para carregar o c�digo do pa�s na tabela de envio no momento da importa��o da venda. Como s� temos venda dentro do Brasil o c�digo � 1058. 
	...
	public NFNotaInfoItemImposto getNFNotaInfoItemImposto(Venda venda, NfeConfig userNfeConfig, int id) {
		...
		// Criar m�todo para retornar a al�quota de imposto do Simples Nacional
...
//				- venda.getEnvio().getCustoComprador()).setScale(2, BigDecimal.ROUND_HALF_EVEN));
	...
	public NFNotaInfoTransportador getNFNotaInfoTransportador() {
		...
		transportador.setRazaoSocial("Mercado Envios Servi�os de Log�stica Ltda");
	...
	public NFNotaInfoVolume getNFNotaInfoVolume() {
		...
		volume.setEspecieVolumesTransportados("Caixa de papel�o");
